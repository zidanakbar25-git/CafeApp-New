            ...
            <div class="mo-layout">
                ...
                <div>
                    ...
                    <div class="mb-3" id="mo-cat-wrap">
                        ...
                        <button class="mo-cat-btn" data-cat-id="<?php echo e($cat->category_id); ?>" data-cat-name="<?php echo e($cat->name); ?>">
                            <?php echo e($cat->name); ?>
                    ...
                    <div class="mb-4" id="mo-sub-wrap">
                        ...
                        <button class="mo-sub-btn" style="display:none;"
                            data-parent-id="<?php echo e($sub->category_id); ?>" data-sub-name="<?php echo e($sub->name); ?>">
                            <?php echo e($sub->name); ?>
                    ...
                    <div class="row g-3" id="mo-menu-grid">
                        ...
                        <div class="col-6 col-lg-4 mo-menu-item"
                            data-cat-name="<?php echo e($menu->category_name); ?>" data-sub-name="<?php echo e($menu->sub_name); ?>">
                            <div class="mo-menu-card">
                                <img src="<?php echo e(asset('storage/' . $menu->image_url)); ?>" alt="<?php echo e($menu->name); ?>">
                                <div class="body">
                                    <div class="mo-menu-name"><?php echo e($menu->name); ?></div>
                                    <div class="mo-menu-price">Rp <?php echo e(number_format($menu->price, 0, ',', '.')); ?></div>
                                    <button class="mo-add-btn add-manual-item"
                                        data-id="<?php echo e($menu->menu_id); ?>"
  ...
  <div class="modal-dialog modal-dialog-centered">
    <div class="modal-content" style="border-radius:20px; border:none;">
      ...
      <div class="modal-body text-center">
        <img src="<?php echo e(asset('images/Qris/Qris.jpeg')); ?>" alt="QRIS" style="width:220px; border-radius:16px;">
...
?>
    ...
    <script>
        ...
        const MO_MENU_OPTIONS = <?php echo json_encode($menus->pluck('option_groups', 'menu_id'), 512) ?>;
        ...
        async function addManualItem(menuId, options) {
            try {
                const res = await fetch('<?php echo e(route("admin.manualOrder.addItem")); ?>', {
                    method: 'POST',
                    headers: {
            ...
            document.querySelectorAll('.mo-qty-btn').forEach(btn => {
                btn.onclick = async function() {
                    ...
                    const res = await fetch('/cart/update-qty-ajax', {
                        ...
                        body: JSON.stringify({
                            detail_id: detailId,
                            order_id: '<?php echo e($order->order_id ?? ""); ?>' || moCurrentOrderId,
                            action
                        }),
                    });
                    const data = await res.json();
    ...
    try {
        const res = await fetch('<?php echo e(route("admin.manualOrder.checkout")); ?>', {
            method: 'POST',
...
document.getElementById('moQrisFinishBtn').addEventListener('click', async function () {
    ...
    try {
        const res = await fetch('<?php echo e(route("admin.manualOrder.qrisFinalize")); ?>', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': CSRF_TOKEN },
        });
        const data = await res.json();
    ...
    `;
        ...
        let moCurrentOrderId = <?php echo json_encode($order ? $order->order_id : null, 15, 512) ?>;

        renderCart(<?php echo json_encode($moOrderItemsForJs, 15, 512) ?>, <?php echo json_encode($order->total_amount ?? 0, 15, 512) ?>);
    </script>

</body>

</html><?php /**PATH C:\laragon\www\newCafeApp\resources\views/admin/cashier/manualorder/index.blade.php ENDPATH**/ ?>